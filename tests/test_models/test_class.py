...
class TestClassDocumentation():
    ...
    def __init__(self, tests, _class):
        ...
        self.function = inspect.getmembers(self.name, inspect.isfunction)
    ...
    def documentation(self):
        ...
        with self.tests.subTest(msg='Testing class'):
            ...
            doc = self.tests.assertGreaterEqual(len(doc), 1)

    def pep8(self, files):
        """Test linter pep8 over the file"""
        pep8style = pep8.StyleGuide(quiet=True)
        result = pep8style.check_files(files)
        self.tests.assertEqual(result.total_errors, 0,
                            "Found cod style errors (and warning).")
